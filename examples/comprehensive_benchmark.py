...
try:
    from uhop.backends.torch_backend import is_torch_available, torch_matmul, torch_conv2d, torch_relu
except:
    is_torch_available = lambda: False

try:
    from uhop.backends.opencl_backend import is_opencl_available, opencl_matmul, opencl_conv2d, opencl_relu
except:
    is_opencl_available = lambda: False

try:
    from uhop.backends.triton_backend import is_triton_available, triton_matmul, triton_relu
except:
    is_triton_available = lambda: False

try:
    from uhop.backends.lite_backend import is_lite_backend_available, lite_matmul, lite_conv2d, lite_relu
except:
...
class BenchmarkResult:
    ...
    def __init__(self, op_name: str, backend: str, size: str, 
                 time_ms: float, gflops: float, error: float, 
            ...
            try:
                ...
                if error_msg:
                    ...
                    self.results.append(BenchmarkResult(
                        "matmul", backend, f"{M}x{K}x{N}", 
                        float('inf'), 0, float('inf'), False, error_msg
                    ))
                    continue
                ...
                self.results.append(BenchmarkResult(
                    "matmul", backend, f"{M}x{K}x{N}",
                    float('inf'), 0, float('inf'), False, str(e)
                ))
        ...
        for backend in backends_to_test:
            ...
            try:
                ...
                if error_msg:
                    print(f"  ✗ {backend:8s} - Failed: {error_msg[:50]}")
                    self.results.append(BenchmarkResult(
                        "conv2d", backend, f"{N}x{C_in}x{H}x{W}",
                        float('inf'), 0, float('inf'), False, error_msg
                    ))
                    continue
...
"""
        ...
        # Add hardware info
        html = html.format(
            device=self.hardware.name or "Unknown",
            memory="N/A",
            compute_units="N/A"
        )
