...
export default function Portal(){
  ...
  const [result, setResult] = useState<{device:string; time:number; timings?: {uhop: number; naive: number; uhop_won: boolean}}|null>(null);
  ...
  useEffect(()=>{
    ...
    async function poll(){
      ...
      }catch(_e){ if(!stop) setAgentConnected(false); }
  ...
  async function connectDevices(){
    ...
    try {
      ...
      setLogs(prev => [...prev, `[Error] Failed to connect: ${error}`].slice(-500));
    } finally {
  ...
  async function genKernel(){
    ...
    try {
      ...
      setLogs(prev => [...prev, `[Error] Kernel generation failed: ${error}`].slice(-500));
    } finally {
      ...
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
          ...
          <section className='md:col-span-3'>
            ...
            >
              <div className='flex items-center justify-between'>
                ...
                <span className='text-[#94a3b8] text-sm'>{showHelp ? 'Hide' : 'Show'}</span>
              </div>
            </button>
            {showHelp && (
              <div className='mt-2 p-3 rounded border border-[#374151] bg-[#0b1220] text-sm space-y-3'>
                <div>
                  This portal can run benchmarks on either the hosted server or your own machine via a lightweight "UHOP Agent".
                </div>
                <div className='space-y-1'>
                  <div className='font-semibold'>Option A — Hosted (no install)</div>
                  <ul className='list-disc pl-5 space-y-1 text-[#9ca3af]'>
                    <li>Click <span className='text-[#e5e7eb]'>Connect Devices</span> to detect the server hardware.</li>
                    <li>Adjust Matrix Size / Iterations if desired.</li>
                    <li>Click <span className='text-[#e5e7eb]'>Run Matmul Test</span> to benchmark on the server.</li>
                    <li>Click <span className='text-[#e5e7eb]'>Generate AI Kernel</span> to view a generated kernel. If the server has no AI key, a fallback kernel is shown.</li>
                  </ul>
                </div>
                <div className='space-y-1'>
                  <div className='font-semibold'>Option B — Local Agent (run on your PC)</div>
                  <ul className='list-disc pl-5 space-y-1 text-[#9ca3af]'>
                    <li>Requirements: Python 3.10+, and optional GPU drivers/toolkits if you want GPU acceleration.</li>
                    <li>Install UHOP: <code className='bg-[#111827] px-1 py-0.5 rounded'>pip install uhop</code> (or from source if developing).</li>
                    <li>Start the agent while this page is open:</li>
                  </ul>
                  <div className='grid md:grid-cols-3 gap-3 text-xs'>
                    <div className='border border-[#374151] rounded p-2'>
                      <div className='text-[#93c5fd] mb-1'>Windows (PowerShell)</div>
                  ...
                  <ul className='list-disc pl-5 space-y-1 text-[#9ca3af] mt-2'>
                    <li>When the agent connects, the status below will show <span className='text-[#6ee7b7]'>Agent: Connected</span>. Then use the same buttons as above — runs will execute on your machine.</li>
                    <li>Optional security: set a token on the server (<code className='bg-[#111827] px-1 py-0.5 rounded'>AGENT_TOKEN</code>) and pass it to the agent (<code className='bg-[#111827] px-1 py-0.5 rounded'>--token</code>).</li>
                    <li>For AI kernel generation with your own API key, set <code className='bg-[#111827] px-1 py-0.5 rounded'>OPENAI_API_KEY</code> in your shell before starting the agent.</li>
                  </ul>
                </div>
                <div className='space-y-1'>
                  <div className='font-semibold'>Notes</div>
                  <ul className='list-disc pl-5 space-y-1 text-[#9ca3af]'>
                    <li>We limit matrix size and iterations in the demo for safety.</li>
                    <li>UHOP chooses backends based on detected hardware (CUDA/OpenCL/CPU/MPS when available).</li>
                    <li>If something fails, check the System Log Console for details.</li>
                  </ul>
                </div>
              </div>
            )}
          ...
          <section className='md:col-span-1 space-y-4'>
            <h3 className='text-lg font-semibold text-[#e5e7eb] mb-3'>Controls</h3>
            ...
            <div className='space-y-3 p-3 rounded border border-[#374151] bg-[#0f172a]'>
              ...
              <div>
                <label className='block text-xs text-[#9ca3af] mb-1'>Matrix Size</label>
                <input 
                  type='number' 
                  value={matrixSize} 
                  onChange={(e) => setMatrixSize(parseInt(e.target.value) || 128)}
                  min='16' 
                  max='512' 
                  step='16'
                  placeholder='128'
              ...
              <div>
                <label className='block text-xs text-[#9ca3af] mb-1'>Iterations</label>
                <input 
                  type='number' 
                  value={iterations} 
            ...
            <div className='space-y-2'>
              ...
              >
                {isRunning ? 'Running...' : `Run Matmul Test (${matrixSize}x${matrixSize})`}
              </button>
              <button 
                className='w-full py-2 rounded bg-[#111827] hover:bg-[#1f2937] border border-[#374151] disabled:opacity-50 disabled:cursor-not-allowed' 
                onClick={genKernel}
            ...
            <div className='rounded border border-[#374151] bg-[#0f172a] p-3 text-xs space-y-2'>
              {result ? (
                <>
                  ...
                  <div>UHOP Time: <span className='text-[#6ee7b7]'>{result.time} ms</span></div>
                  {result.timings && (
                    <>
                      <div className='border-t border-[#374151] pt-2 mt-2'>
                        <div className='text-[#9ca3af] text-xs mb-1'>Detailed Timings:</div>
                        <div>UHOP: <span className='text-[#6ee7b7]'>{(result.timings.uhop * 1000).toFixed(2)} ms</span></div>
                        <div>Naive: <span className='text-[#fca5a5]'>{(result.timings.naive * 1000).toFixed(2)} ms</span></div>
                        <div>Speedup: <span className='text-[#fbbf24]'>{(result.timings.naive / result.timings.uhop).toFixed(1)}x</span></div>
                        <div className={`mt-1 ${result.timings.uhop_won ? 'text-[#6ee7b7]' : 'text-[#fca5a5]'}`}>
                          {result.timings.uhop_won ? '✅ UHOP Wins!' : '⚠️ Baseline was faster'}
                        </div>
                      </div>
                    </>
                  )}
                </>
              ) : <div className='text-[#64748b]'>Run a benchmark to see results</div>}
            </div>
          </section>
          ...
          <section className='md:col-span-2'>
            ...
            <pre className='rounded border border-[#374151] bg-[#0b1220] p-3 text-xs overflow-auto max-h-64'>
              <code className='text-[#e5e7eb]'>
                {code || '// Click "Generate AI Kernel" to view an AI-generated CUDA/OpenCL kernel\n// Real UHOP AI will generate optimized kernels based on your hardware'}
              </code>
            </pre>
          </section>
        </div>
