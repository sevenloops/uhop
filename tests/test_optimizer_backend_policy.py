...
def test_gpu_first_policy_cpu_only(monkeypatch):
    ...
    monkeypatch.setattr(
        "uhop.backends.triton_backend._TRITON_AVAILABLE", False, raising=False
    )
    monkeypatch.setattr(
        "uhop.backends.opencl_backend._OPENCL_AVAILABLE", False, raising=False
    )
    from uhop.backends import torch_backend

    monkeypatch.setattr(
        torch_backend, "_torch_device_preference", lambda: None, raising=False
    )
