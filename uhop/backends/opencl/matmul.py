...
class MatmulOp:
    def __init__(self):
        ...
        self._force_naive = str(os.environ.get("UHOP_OPENCL_FORCE_NAIVE") or _cfg.get("UHOP_OPENCL_FORCE_NAIVE") or "0").lower() in ("1","true","yes","on")

    # --- Tuning helpers -------------------------------------------------
    def _tune_tiled(self, ctx, q, A: _np.ndarray, B: _np.ndarray,
                    N: int, M: int, K: int, dev_name: str, shape_key: str,
                    flip: bool) -> Optional[dict]:
