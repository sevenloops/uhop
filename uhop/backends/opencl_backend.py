...
def is_opencl_available() -> bool:
    # Allow forcing baseline mode to skip OpenCL even if pyopencl is importable
    if os.environ.get("UHOP_FORCE_BASELINE") == "1":
        return False
...
if _OPENCL_AVAILABLE:
    ...
    def _choose_conv_impl(ctx, N, C, H, W, Cout, KH, KW, stride, padding) -> str:
        ...
        try:
            ...
            auto = _UhopAutotune()
            params = (
                auto.get_params("opencl", "clblast", "sgemm", str(dev_name), "device")
                or {}
            )
        ...
        if (
            "nvidia" in vendor or "advanced micro devices" in vendor or "amd" in vendor
        ) and (ksz >= 128 and outHW >= 128):
            return "im2col"
        # Default: tiled
        return "tiled"
    ...
    def _load_im2col_source() -> str:
        ...
        src_path = (
            _Path(__file__).resolve().parents[1] / "kernels" / "opencl" / "im2col.cl"
        )
        return src_path.read_text()

    def _get_program(
        ctx, source: str, program_key: str, build_options: Optional[str] = None
    ):
        """
    ...
    def opencl_matmul(a, b):
        ...
        if impl == "clblast":
            try:
                from .clblast_integration import load_clblast
                from .clblast_integration import sgemm as _clblast_sgemm
                ...
                if lib is not None:
                    ...
                    try:
                        ...
                        params = (
                            auto.get_params(
                                "opencl", "clblast", "sgemm", str(dev_name), "device"
                            )
                            or {}
                        )
                        if bool(params.get("unstable")):
                            warnings.warn(
                                "UHOP: CLBlast marked unstable on this device; using tiled GEMM."
                            )
                    ...
                    if status != 0:
                        warnings.warn(
                            f"UHOP: CLBlastSgemm failed with status {status}; falling back to tiled GEMM."
                        )
                        try:
                            from ..cache import UhopAutotune as _UhopAutotune

                            dev_name = ctx.devices[0].name if ctx.devices else "unknown"
                            _UhopAutotune().set_params(
                                "opencl",
                                "clblast",
                                "sgemm",
                                str(dev_name),
                    ...
                    warnings.warn(
                        "UHOP: CLBlast library not found; falling back to tiled GEMM."
                    )
            except Exception as e:
                warnings.warn(
                    f"UHOP: CLBlast path error: {e}; falling back to tiled GEMM."
                )
        ...
        params = (
            auto.get_params("opencl", "matmul", "matmul_tiled", dev_name, shape_key)
            or {}
        )
        ...
        for cand in candidates:
            ...
            try:
                ...
                prg = _get_program(
                    ctx, src, f"matmul_tiled_T{tile}_V{vec}", build_options=build_opts
                )
                ...
                lsz = (tile, tile)
                kn.set_args(
                    a_buf, b_buf, c_buf, _np.int32(m), _np.int32(k), _np.int32(n)
                )
                evt = cl.enqueue_nd_range_kernel(q, kn, gsz, lsz)
                evt.wait()
                try:
        ...
        if best and best_tile:
            try:
                ...
                auto.set_params(
                    ...
                    dev_name,
                    shape_key,
                    {"tile": int(best_tile), "vec": int(best_vec)},
                )
            except Exception:
                pass
        ...
        prg = _get_program(
            ctx,
            src,
            f"matmul_tiled_T{best_tile}_V{best_vec}",
            build_options=f"-D TILE={int(best_tile)} -D VEC={int(best_vec)}",
        )
        kn = cl.Kernel(prg, "matmul_tiled")
    ...
    def opencl_conv2d(
        ...
        if stride == 1 and padding == 0 and N == 1:
            ...
            if fuse_relu:
                ...
                prg = _get_program(
                    ctx,
                    _load_conv2d_relu_kernel_source(),
            ...
            if impl in ("im2col_gemm", "im2col"):
                ...
                if lib is not None:
                    ...
                    try:
                        ...
                        if bool(params.get("unstable")):
                            warnings.warn(
                                "UHOP: CLBlast marked unstable on this device; using tiled conv2d."
                            )
                            lib = None
                    except Exception:
                        pass
                    ...
                    for b_idx in range(int(N)):
                        ...
                        try:
                            ...
                            if status != 0:
                                warnings.warn(
                                    f"UHOP: CLBlastSgemm (im2col) failed with status {status}; using tiled conv2d."
                                )
                                try:
                                    from ..cache import \
                                        UhopAutotune as _UhopAutotune
                            ...
                            warnings.warn(
                                f"UHOP: CLBlastSgemm (im2col) raised {e}; using tiled conv2d."
                            )
                            try:
                                from ..cache import \
                                    UhopAutotune as _UhopAutotune
                    ...
                    if lib is not None:
                        if fuse_relu:
                            out = opencl_relu(out.reshape(-1)).reshape(
                                N, Cout, outH, outW
                            )
                        return out
                else:
                    warnings.warn(
                        "UHOP: CLBlast not available for im2col+GEMM; using tiled conv2d."
                    )
            ...
            dev_name = ctx.devices[0].name if ctx.devices else "unknown"
            shape_key = (
                f"N{N}_C{C}_H{H}_W{W}_Co{Cout}_KH{KH}_KW{KW}_S{stride}_P{padding}"
            )
            auto = _UhopAutotune()
            lsz_t = auto.get_lsz(
                "opencl", "conv2d", "conv2d_tiled", dev_name, shape_key
            )
            ...
            if lsz_t and len(lsz_t) in (2, 3):
                ...
                candidates = [
                    (tx, ty, tz, v) for (tx, ty, tz) in base_tiles for v in vec_cands
                ]
            ...
            for cand in candidates:
                ...
                try:
                    ...
                    prg = _get_program(
                        ctx,
                        src_conv,
                        f"conv2d_tiled_T{tx}x{ty}_V{vec}",
            ...
            prg = _get_program(
                ctx,
                src_conv,
                f"conv2d_tiled_T{int(tx)}x{int(ty)}_V{int(vec)}",
                build_options=build_opts,
            )
            kn = cl.Kernel(prg, "conv2d_tiled")
    ...
    def opencl_conv2d_backward_input(
        ...
        outH = int((H + 2 * padding - KH) // stride + 1)
        outW = int((W + 2 * padding - KW) // stride + 1)
        assert grad_out_np.shape == (
            N,
            Cout,
            outH,
            outW,
        ), f"grad_out shape {grad_out_np.shape} mismatch expected {(N, Cout, outH, outW)}"
