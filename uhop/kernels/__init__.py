...
class KernelRegistry:
    ...
    def _register_cuda_kernels(self):
        ...
        kernels = [
            ...
            ("einsum", "einsum.cu", ["einsum_matmul_kernel", "einsum_bmm_kernel", "einsum_tensor_contract_kernel"]),
            ...
            ("scaled_dot_product_attention", "attention.cu", ["scaled_dot_product_attention_kernel", "attention_softmax_kernel", "attention_output_kernel"]),
            ...
            ("fused_add_layernorm_gelu", "fused_add_norm_act.cu", ["fused_add_layernorm_gelu_kernel"]),
            ("fused_add_layernorm_relu", "fused_add_norm_act.cu", ["fused_add_layernorm_relu_kernel"]),
            ("fused_add_layernorm_silu", "fused_add_norm_act.cu", ["fused_add_layernorm_silu_kernel"]),
        ]
    ...
    def _register_opencl_kernels(self):
        ...
        kernels = [
            ...
            ("reduce_mean", "reduce.cl", ["reduce_sum_partials", "reduce_mean_finalize"]),
            ("reduce_max", "reduce.cl", ["reduce_max_partials", "reduce_max_finalize"]),
            ("reduce_min", "reduce.cl", ["reduce_min_partials", "reduce_min_finalize"]),
            ("reduce_norm", "reduce.cl", ["reduce_norm_partials", "reduce_norm_finalize"]),
        ]
        
        for op_name, filename, kernel_names in kernels:
            self._register(op_name, BackendType.OPENCL, opencl_dir / filename, kernel_names)
    
    def _register(self, operator: str, backend: BackendType, file_path: Path, kernel_names: List[str]):
