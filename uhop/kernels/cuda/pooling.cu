// uhop/kernels/cuda/pooling.cu
// Max and Average Pooling kernels

extern "C" __global__ void maxpool2d_kernel(const float *input, float *output,
...
avgpool2d_kernel(const float *input, float *output, int batch_size,
                 int channels, int input_h, int input_w, int kernel_h,
                 int kernel_w, int stride_h, int stride_w, int padding_h,
                 int padding_w, bool count_include_pad) {
  int output_h = (input_h + 2 * padding_h - kernel_h) / stride_h + 1;
  int output_w = (input_w + 2 * padding_w - kernel_w) / stride_w + 1;
  ...
  output[idx] = sum / (count > 0 ? count : 1);
}
