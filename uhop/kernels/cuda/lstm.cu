...
lstm_cell_kernel(const float *input, const float *hidden, const float *cell,
                 const float *weight_ih, const float *weight_hh,
                 const float *bias_ih, const float *bias_hh,
                 float *output_hidden, float *output_cell, int batch_size,
  ...
  for (int gate = 0; gate < 4; ++gate) {
    ...
    if (bias_ih)
      sum += bias_ih[gate * hidden_size + h];
    ...
    if (bias_hh)
      sum += bias_hh[gate * hidden_size + h];
