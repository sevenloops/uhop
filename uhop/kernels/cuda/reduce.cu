...
extern "C" __global__ void reduce_sum_atomic(const float *X, float *Out,
                                             int N) {
...
extern "C" __global__ void reduce_max_atomic(const float *X, float *Out,
                                             int N) {
...
extern "C" __global__ void reduce_min_atomic(const float *X, float *Out,
                                             int N) {
...
extern "C" __global__ void reduce_norm_atomic(const float *X, float *Out,
                                              int N) {
  __shared__ float block_sum;
  if (threadIdx.x == 0)
    block_sum = 0.0f;
