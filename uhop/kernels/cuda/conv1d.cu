...
conv1d_kernel(const float *input, const float *weight, const float *bias,
  ...
  if (b >= batch_size || oc >= out_channels || out_idx >= output_length)
    return;
  ...
  for (int ic = 0; ic < in_channels; ++ic) {
    for (int k = 0; k < kernel_size; ++k) {
      ...
      if (in_idx >= 0 && in_idx < input_length) {
        float in_val =
            input[b * in_channels * input_length + ic * input_length + in_idx];
        float w_val =
