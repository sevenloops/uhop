...
batchnorm_kernel(const float *input, const float *gamma, const float *beta,
...
extern "C" __global__ void
batchnorm_2d_kernel(const float *input, const float *gamma, const float *beta,
                    const float *running_mean, const float *running_var,
                    float *output, int batch_size, int channels, int height,
                    int width, float eps) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
