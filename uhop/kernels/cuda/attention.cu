...
extern "C" __global__ void scaled_dot_product_attention_kernel(
    const float *Q, const float *K, const float *V, float *output,
    float *scores, int batch_size, int num_heads, int seq_len, int head_dim,
...
extern "C" __global__ void attention_softmax_kernel(float *scores,
                                                    int batch_size,
                                                    int num_heads,
                                                    int seq_len) {
  int b = blockIdx.z / num_heads;
...
attention_output_kernel(const float *scores, const float *V, float *output,
  ...
  if (b >= batch_size || i >= seq_len || d >= head_dim)
    return;
