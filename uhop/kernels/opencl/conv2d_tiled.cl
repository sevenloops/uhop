...
// Assumes small kernels (e.g., 3x3, 5x5). Uses dynamic local memory passed as
// kernel args.
...
__kernel void conv2d_tiled(__global const float *input,  // [B,C,H,W]
                           ...
                           const int outH, const int outW, const int stride,
                           const int pad, __local float *tile_in,
  ...
  if (b >= B)
    return; // z dimension matches exactly N*K, but keep safety.
