...
def bench_elementwise(ctx: Any, q: Any, kernels_path: Path, op: str, N: int, warmup: int, iters: int, validate: bool=False) -> BenchRecord:
    ...
    unary_ops = {"elementwise_exp", "elementwise_log", "elementwise_sqrt", "elementwise_leakyrelu"}
    ...
    if op in {"elementwise_add", "elementwise_sub", "elementwise_mul", "elementwise_min", "elementwise_max"}:
        flops = float(N)
    elif op == "elementwise_leakyrelu":
        flops = float(2 * N)
    else:
...
def bench_activations(ctx: Any, q: Any, kernels_path: Path, op: str, N: int, warmup: int, iters: int, validate: bool=False, compare_optimize: bool=False) -> BenchRecord:
    ...
    if validate and base_fn is not None:
        ...
        ref = base_fn(); ref = np.asarray(ref, dtype=np.float32)
        diff = np.abs(out_host - ref)
        vmax = float(np.max(diff))
        denom = np.maximum(np.abs(ref), 1e-6)
        vrmax = float(np.max(diff / denom))
    ...
    if compare_optimize and op == "relu":
        try:
            ...
            for _ in range(max(1, iters)):
                t0 = time.perf_counter(); _ = _naive_relu(x.copy()); mm.append((time.perf_counter()-t0)*1000.0)
            uhop_mean = float(statistics.mean(mm))
            uhop_spd = (baseline["mean"] / uhop_mean) if (baseline.get("mean") and uhop_mean and uhop_mean > 0) else None
        except Exception:
            uhop_mean = None
            uhop_spd = None

    return BenchRecord(op=op, shape=(N,), mean_ms=stats["mean"], median_ms=stats["median"], min_ms=stats["min"], max_ms=stats["max"], std_ms=stats["std"], iters=iters, baseline_mean_ms=baseline.get("mean"), speedup_vs_baseline=spd, validated=vpass, max_abs_err=vmax, max_rel_err=vrmax, uhop_mean_ms=uhop_mean, uhop_speedup_vs_naive=uhop_spd)


def bench_reduce_sum(ctx: Any, q: Any, kernels_path: Path, N: int, warmup: int, iters: int, validate: bool=False) -> BenchRecord:
    ...
    return BenchRecord(op="reduce_sum", shape=(N,), mean_ms=mean_ms, median_ms=median_ms, min_ms=stats["min"], max_ms=stats["max"], std_ms=stats["std"], iters=iters*2, baseline_mean_ms=baseline.get("mean"), speedup_vs_baseline=spd, validated=vpass, max_abs_err=vmax, max_rel_err=vrmax)


def bench_matmul(ctx: Any, q: Any, kernels_path: Path, N: int, M: int, K: int, warmup: int, iters: int, validate: bool=False, compare_optimize: bool=False, kernel_variant: str = "tiled") -> BenchRecord:
    ...
    if compare_optimize:
        try:
            ...
            import time
            for _ in range(max(1, iters)):
                t0 = time.perf_counter(); _ = _naive(A, B); mm.append((time.perf_counter()-t0)*1000.0)
            uhop_mean = float(statistics.mean(mm))
            uhop_spd = (baseline["mean"] / uhop_mean) if (baseline.get("mean") and uhop_mean and uhop_mean > 0) else None
        except Exception:
            uhop_mean = None
...
def bench_conv2d(ctx: Any, q: Any, kernels_path: Path,
                 B: int, C: int, H: int, W: int, K: int, R: int, S: int, stride: int, pad: int,
                 warmup: int, iters: int, validate: bool=False, compare_optimize: bool=False) -> BenchRecord:
    ...
    def launch():
        return prg.conv2d(q, gsz, None,
                          inp_buf, w_buf, out_buf,
                          np.int32(B), np.int32(C), np.int32(H), np.int32(W),
                          np.int32(K), np.int32(R), np.int32(S),
                          np.int32(outH), np.int32(outW), np.int32(stride), np.int32(pad))
    ...
    if validate:
        ...
        cl.enqueue_copy(q, out_host, out_buf).wait()
        ref = np.asarray(torch.nn.functional.conv2d(inp_t, w_t, stride=stride, padding=pad).numpy(), dtype=np.float32)
        diff = np.abs(out_host - ref)
        vmax = float(np.max(diff))
        denom = np.maximum(np.abs(ref), 1e-6)
        vrmax = float(np.max(diff / denom))
    ...
    if compare_optimize:
        try:
            ...
            import time
            for _ in range(max(1, iters)):
                t0 = time.perf_counter(); _ = _naive(inp_t, w_t, stride=stride, padding=pad); mm.append((time.perf_counter()-t0)*1000.0)
            uhop_mean = float(statistics.mean(mm))
...
def bench_conv2d_relu(B: int, C: int, H: int, W: int, K: int, R: int, S: int, stride: int, pad: int, warmup: int, iters: int, validate: bool=False) -> BenchRecord:
    ...
    import time
    import torch
    from uhop.backends.opencl_backend import opencl_conv2d, opencl_conv2d_relu, opencl_relu
    ...
    if validate:
        ...
        w_t = torch.from_numpy(w)
        ref = torch.nn.functional.relu(torch.nn.functional.conv2d(inp_t, w_t, stride=stride, padding=pad)).numpy().astype(np.float32)
        diff = np.abs(yf - ref)
        vmax = float(np.max(diff))
        denom = np.maximum(np.abs(ref), 1e-6)
        vrmax = float(np.max(diff / denom))
...
def bench_conv2d_input_grad(B: int, C: int, H: int, W: int, K: int, R: int, S: int, stride: int, pad: int, warmup: int, iters: int, validate: bool=False) -> BenchRecord:
    import torch
    import time
    ...
    return BenchRecord(op="conv2d_input_grad", shape=(B, C, H, W, K, R, S, stride, pad), mean_ms=stats["mean"], median_ms=stats["median"], min_ms=stats["min"], max_ms=stats["max"], std_ms=stats["std"], iters=iters, baseline_mean_ms=base.get("mean"), speedup_vs_baseline=spd, gflops=gflops, validated=vpass, max_abs_err=vmax, max_rel_err=vrmax)


def bench_conv2d_weight_grad(B: int, C: int, H: int, W: int, K: int, R: int, S: int, stride: int, pad: int, warmup: int, iters: int, validate: bool=False) -> BenchRecord:
    import torch
    import time
    ...
    return BenchRecord(op="conv2d_weight_grad", shape=(B, C, H, W, K, R, S, stride, pad), mean_ms=stats["mean"], median_ms=stats["median"], min_ms=stats["min"], max_ms=stats["max"], std_ms=stats["std"], iters=iters, baseline_mean_ms=base.get("mean"), speedup_vs_baseline=spd, gflops=gflops, validated=vpass, max_abs_err=vmax, max_rel_err=vrmax)


def bench_softmax(ctx: Any, q: Any, kernels_path: Path, batch: int, classes: int, warmup: int, iters: int, log: bool, validate: bool=False) -> BenchRecord:
    ...
    return BenchRecord(op="logsoftmax" if log else "softmax", shape=(batch, classes), mean_ms=stats["mean"], median_ms=stats["median"], min_ms=stats["min"], max_ms=stats["max"], std_ms=stats["std"], iters=iters, baseline_mean_ms=base.get("mean"), speedup_vs_baseline=spd, validated=vpass, max_abs_err=vmax, max_rel_err=vrmax)


def bench_pool2d(ctx: Any, q: Any, kernels_path: Path, op: str, B: int, C: int, H: int, W: int, k: int, stride: int, pad: int, warmup: int, iters: int, validate: bool=False) -> BenchRecord:
    ...
    if op == "maxpool2d":
        def launch():
            return prg.maxpool2d_kernel(q, (total,), None, inp_buf, out_buf,
                                        np.int32(B), np.int32(C), np.int32(H), np.int32(W),
                                        np.int32(k), np.int32(k), np.int32(stride), np.int32(stride),
                                        np.int32(pad), np.int32(pad))
    elif op == "avgpool2d":
        def launch():
            return prg.avgpool2d_kernel(q, (total,), None, inp_buf, out_buf,
                                        np.int32(B), np.int32(C), np.int32(H), np.int32(W),
                                        np.int32(k), np.int32(k), np.int32(stride), np.int32(stride),
                                        np.int32(pad), np.int32(pad), np.int32(0))
    else:
    ...
    def baseline():
        if op == "maxpool2d":
            ...
            torch.nn.functional.avg_pool2d(inp_t, kernel_size=k, stride=stride, padding=pad, ceil_mode=False, count_include_pad=False)
    base = _measure_host_callable_ms(baseline, warmup=1, iters=max(1, iters))
    spd = (base["mean"] / stats["mean"]) if (base.get("mean") and stats["mean"] > 0) else None
    ...
    if validate:
        ...
        if op == "maxpool2d":
            ...
            ref = torch.nn.functional.avg_pool2d(inp_t, kernel_size=k, stride=stride, padding=pad, ceil_mode=False, count_include_pad=False)
    ...
    return BenchRecord(op=op, shape=(B, C, H, W, k, stride, pad), mean_ms=stats["mean"], median_ms=stats["median"], min_ms=stats["min"], max_ms=stats["max"], std_ms=stats["std"], iters=iters, baseline_mean_ms=base.get("mean"), speedup_vs_baseline=spd, validated=vpass, max_abs_err=vmax, max_rel_err=vrmax)


def bench_depthwise_conv2d(ctx: Any, q: Any, kernels_path: Path,
                           B: int, C: int, H: int, W: int, KH: int, KW: int, stride: int, pad: int,
                           warmup: int, iters: int, validate: bool=False) -> BenchRecord:
    ...
    def launch():
        return prg.depthwise_conv2d(q, gsz, None,
                                    inp_buf, w_buf, b_buf, out_buf,
                                    np.int32(B), np.int32(C), np.int32(H), np.int32(W),
                                    np.int32(KH), np.int32(KW),
                                    np.int32(stride), np.int32(stride),
                                    np.int32(pad), np.int32(pad))
    ...
    if validate:
        out_host = np.empty_like(out)
        cl.enqueue_copy(q, out_host, out_buf).wait()
        ref = torch.nn.functional.conv2d(inp_t, w_t, b_t, stride=stride, padding=pad, groups=C).numpy().astype(np.float32)
        diff = np.abs(out_host - ref)
        vmax = float(np.max(diff))
        denom = np.maximum(np.abs(ref), 1e-6)
        vrmax = float(np.max(diff / denom))
...
def bench_groupnorm(ctx: Any, q: Any, kernels_path: Path,
                    B: int, C: int, H: int, W: int, groups: int, eps: float,
                    warmup: int, iters: int, validate: bool=False) -> BenchRecord:
    ...
    def launch():
        return prg.group_norm(q, gsz, None, X_buf, g_buf, b_buf, Y_buf,
                              np.int32(B), np.int32(C), np.int32(H), np.int32(W),
                              np.int32(g), np.float32(eps))
...
def main():
    ...
    parser.add_argument("--ops", type=str, default="all", help="Comma-separated ops or 'all' for a curated set")
    parser.add_argument("--warmup", type=int, default=3)
    parser.add_argument("--iterations", type=int, default=10)
    parser.add_argument("--size", type=int, default=1<<20, help="Vector length for elementwise/reduce ops")
    parser.add_argument("--matmul-shape", type=str, default="512,512,512", help="N,M,K for matmul")
    parser.add_argument("--matmul-kernel", type=str, default="tiled", choices=["tiled", "naive"], help="Matmul kernel variant to use")
    parser.add_argument("--conv-shape", type=str, default="8,32,112,112,64,3,3,1,1", help="B,C,H,W,K,R,S,stride,pad for conv2d")
    parser.add_argument("--conv-shapes", type=str, default=None, help="Semicolon-separated list of conv2d shapes for batch compare, e.g., '8,32,112,112,64,3,3,1,1;8,64,56,56,64,3,3,1,1'")
    parser.add_argument("--softmax-shape", type=str, default="1024,1000", help="batch,classes for softmax")
    parser.add_argument("--pool-shape", type=str, default="8,32,112,112,3,2,1", help="B,C,H,W,k,stride,pad for pool2d ops")
    parser.add_argument("--depthwise-shape", type=str, default="8,32,112,112,3,3,1,1", help="B,C,H,W,KH,KW,stride,pad for depthwise_conv2d")
    parser.add_argument("--groupnorm-shape", type=str, default="8,32,56,56", help="B,C,H,W for group_norm")
    parser.add_argument("--groupnorm-groups", type=int, default=8, help="Number of groups for group_norm")
    parser.add_argument("--groupnorm-eps", type=float, default=1e-5, help="Epsilon for group_norm")
    parser.add_argument("--validate", action="store_true", help="Validate device outputs vs host baselines")
    parser.add_argument("--compare-optimize", action="store_true", help="Compare @uhop.optimize ops vs naive (supported ops only)")
    ...
    for op in ops:
        ...
        if op.startswith("elementwise_"):
            results.append(bench_elementwise(ctx, q, kpath, op, N, args.warmup, args.iterations, validate=args.validate))
        elif op in ("relu", "gelu", "silu"):
            results.append(bench_activations(ctx, q, kpath, op, N, args.warmup, args.iterations, validate=args.validate))
        elif op == "reduce_sum":
            results.append(bench_reduce_sum(ctx, q, kpath, N, args.warmup, args.iterations, validate=args.validate))
        elif op == "matmul":
            results.append(bench_matmul(ctx, q, kpath, Nmm, Mmm, Kmm, args.warmup, args.iterations, validate=args.validate, compare_optimize=args.compare_optimize, kernel_variant=args.matmul_kernel))
        elif op == "conv2d":
            results.append(bench_conv2d(ctx, q, kpath, Bc, Cc, Hc, Wc, Kc, Rc, Sc, stridec, padc, args.warmup, args.iterations, validate=args.validate, compare_optimize=args.compare_optimize))
        elif op == "conv2d_relu":
            results.append(bench_conv2d_relu(Bc, Cc, Hc, Wc, Kc, Rc, Sc, stridec, padc, args.warmup, args.iterations, validate=args.validate))
            ...
            for (b, c, h, w, k, r, s, st, pd) in shapes:
                rec = bench_conv2d_relu(b, c, h, w, k, r, s, st, pd, args.warmup, args.iterations, validate=args.validate)
                results.append(rec)
                compare_rows.append({
            ...
            results.append(bench_conv2d_input_grad(Bc, Cc, Hc, Wc, Kc, Rc, Sc, stridec, padc, args.warmup, args.iterations, validate=args.validate))
        elif op == "conv2d_weight_grad":
            results.append(bench_conv2d_weight_grad(Bc, Cc, Hc, Wc, Kc, Rc, Sc, stridec, padc, args.warmup, args.iterations, validate=args.validate))
        elif op == "softmax":
            results.append(bench_softmax(ctx, q, kpath, Sb, Cb, args.warmup, args.iterations, False, validate=args.validate))
        elif op == "logsoftmax":
            results.append(bench_softmax(ctx, q, kpath, Sb, Cb, args.warmup, args.iterations, True, validate=args.validate))
        elif op in ("maxpool2d", "avgpool2d"):
            results.append(bench_pool2d(ctx, q, kpath, op, Bp, Cp, Hp, Wp, kp, sp, pp, args.warmup, args.iterations, validate=args.validate))
        elif op == "depthwise_conv2d":
            results.append(bench_depthwise_conv2d(ctx, q, kpath, Bd, Cd, Hd, Wd, KHd, KWd, sd, pd, args.warmup, args.iterations, validate=args.validate))
        elif op == "group_norm":
            results.append(bench_groupnorm(ctx, q, kpath, Bg, Cg, Hg, Wg, Gg, Eg, args.warmup, args.iterations, validate=args.validate))
        else:
    ...
    for r in results:
        ...
        print(f"- {r.op:14s} shape={r.shape}  mean={r.mean_ms:8.3f} ms  (±{r.std_ms:6.3f} ms){base}{spd}{perf}{val}{uh}")
