...
from .metrics import load_kpi_snapshot, write_kpi_snapshot
...
def main() -> int:
    ...
    ap.add_argument(
        "--output", "-o", default=str(Path.home() / ".uhop_mvp_cache" / "metrics.json"), help="Output JSON path"
    )
    ...
    if args.show:
                ...
                display_rows.append(
                    {
                        ...
                        "gflops": (
                            f"{r.get('last_gflops'):.2f}" if isinstance(r.get("last_gflops"), (int, float)) else "-"
                        ),
                    }
                )
        ...
        if oclc:
            ...
            print(
                _fmt_table(
                    display_rows,
                    ["shape", "kernel", "ms", "im2col", "gemm", "copy", "chunk", "chunks", "var", "retune"],
                )
            )
    return 0
